...
export GIT_TERMINAL_PROMPT=0   # never prompt for git credentials — fail fast so we can offer a deploy key
...
SSH_CMD="${GIT_SSH_COMMAND:-ssh -o StrictHostKeyChecking=accept-new -o BatchMode=yes -o ConnectTimeout=20}"
DEPLOY_KEY=""   # per-repo deploy key (generated by the guided flow; reused on re-run)
# reuse a deploy key minted on a prior install (so a re-run authenticates without re-prompting)
if [ -z "${GIT_SSH_COMMAND:-}" ] && [ -f "$STATE_DIR/deploy_key" ]; then
  DEPLOY_KEY="$STATE_DIR/deploy_key"
  SSH_CMD="ssh -i $DEPLOY_KEY -o StrictHostKeyChecking=accept-new -o BatchMode=yes -o ConnectTimeout=20"
fi
...
resolve_remote() {            # sets REMOTE_MODE (local|remote), REMOTE_HAS_COMMITS, REMOTE_URL
  ...
  if out="$(GIT_SSH_COMMAND="$SSH_CMD" git ls-remote "$REMOTE_URL" 2>&1)"; then
    ...
    log "can't reach $REMOTE_URL yet — let's set up a deploy key for this box."
    _guided_deploy_key
    out="$(GIT_SSH_COMMAND="$SSH_CMD" git ls-remote "$REMOTE_URL" 2>&1)" || die "remote still unreachable: $REMOTE_URL"
  else
    die "can't reach or authenticate the remote: $REMOTE_URL
  $out
  Give this box git push access (e.g. add a deploy key with WRITE access — an interactive
  re-run generates one and walks you through it), or pass --no-remote."
  fi
  REMOTE_MODE=remote
  if [ -n "$out" ]; then
...
_ssh_url_from() {             # a github https URL -> its git@ form; everything else unchanged
  case "$1" in
    https://github.com/*) local p="${1#https://github.com/}"; p="${p%.git}"; p="${p%/}"; echo "git@github.com:${p}.git" ;;
    *) echo "$1" ;;
  esac
}

# Interactive only: mint a per-repo SSH deploy key, walk the admin through adding it,
# and retry until the remote is reachable (or they skip to do their own auth). A deploy
# key is the least-privilege fit — one repo, this box, no expiry.
_guided_deploy_key() {
  mkdir -p "$STATE_DIR"
  local sshu; sshu="$(_ssh_url_from "$REMOTE_URL")"
  if [ "$sshu" != "$REMOTE_URL" ]; then
    log "deploy keys use SSH — switching the remote to $sshu"; REMOTE_URL="$sshu"
  elif [ "${REMOTE_URL#https://}" != "$REMOTE_URL" ]; then
    log "NOTE: deploy keys need an SSH URL (git@host:owner/repo.git); re-run with the SSH URL if the key doesn't take."
  fi
  DEPLOY_KEY="$STATE_DIR/deploy_key"
  if [ ! -f "$DEPLOY_KEY" ]; then
    ssh-keygen -t ed25519 -N "" -C "know-${NAME:-brain}@$(hostname 2>/dev/null || echo box)" -f "$DEPLOY_KEY" -q
    chmod 600 "$DEPLOY_KEY"; log "generated a dedicated deploy key at $DEPLOY_KEY"
  fi
  SSH_CMD="ssh -i $DEPLOY_KEY -o StrictHostKeyChecking=accept-new -o BatchMode=yes -o ConnectTimeout=20"
  printf '\n\033[1;33m[know] Add this box as a deploy key WITH WRITE ACCESS so it can push the KB:\033[0m\n\n'
  cat "$DEPLOY_KEY.pub"; printf '\n'
  case "$REMOTE_URL" in
    git@github.com:*)      local pr="${REMOTE_URL#git@github.com:}"; pr="${pr%.git}"
      printf '  GitHub: https://github.com/%s/settings/keys/new — paste the key, CHECK "Allow write access".\n\n' "$pr" ;;
    https://github.com/*)  local pr="${REMOTE_URL#https://github.com/}"; pr="${pr%.git}"; pr="${pr%/}"
      printf '  GitHub: https://github.com/%s/settings/keys/new — paste the key, CHECK "Allow write access".\n\n' "$pr" ;;
    *) printf '  Add the key above as a deploy/access key WITH WRITE ACCESS on your git host.\n\n' ;;
  esac
  while true; do
    printf '\033[1;36m[know]\033[0m Press Enter to retry once added, or type "skip" to set up auth yourself: '
    local ans; read -r ans || ans=skip
    case "$ans" in
      skip|SKIP|s) die "skipped — set up this box's git push auth yourself (or use --no-remote), then re-run." ;;
      *) if GIT_SSH_COMMAND="$SSH_CMD" git ls-remote "$REMOTE_URL" >/dev/null 2>&1; then
           ok "deploy key works — remote reachable"; return 0
         fi
         log "still can't reach $REMOTE_URL — is the key on THAT repo with WRITE access? try again, or 'skip'." ;;
    esac
  done
}
...
setup_kb() {
  ...
  # persist the deploy key into the repo so the gateway's runtime push/pull use it too
  # (repo-local core.sshCommand applies to every git op on this repo — no env needed).
  if [ -n "$DEPLOY_KEY" ] && [ -d "$KB_REPO/.git" ]; then
    git -C "$KB_REPO" config core.sshCommand "ssh -i $DEPLOY_KEY -o StrictHostKeyChecking=accept-new -o BatchMode=yes"
  fi
  if [ "$REMOTE_MODE" = remote ]; then
